...
function getCssSelector(el: HTMLElement): string {
  ...
  while (current && current !== document.body) {
    ...
    if (current.className && typeof current.className === "string") {
      const classes = current.className.split(/\s+/).filter((c) => c && !c.startsWith("hover:") && c.length < 30).slice(0, 2)
...
export function PublicFeedbackButton() {
  ...
  useEffect(() => {
    ...
    overlay.style.cssText = `position: fixed; pointer-events: none; z-index: 99999; border: 2px solid hsl(var(--primary)); background: hsl(var(--primary) / 0.08); border-radius: 4px; transition: all 0.1s ease; display: none;`
    document.body.appendChild(overlay)
    const tooltip = document.createElement("div")
    tooltip.style.cssText = `position: fixed; z-index: 100000; pointer-events: none; background: hsl(var(--popover)); color: hsl(var(--popover-foreground)); font-size: 11px; padding: 4px 8px; border-radius: 4px; display: none; max-width: 200px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; border: 1px solid hsl(var(--border));`
              ...
              <CollapsibleSection title="Targeted Element" icon={Crosshair} badge={targetedElement.tagName}>
                <div className="space-y-2">
                  <div className="space-y-1 text-xs text-muted-foreground">
                    <p className="break-all"><span className="font-medium">Selector:</span> <code className="rounded bg-muted px-1 text-xs break-all">{targetedElement.selector}</code></p>
                    <p><span className="font-medium">Tag:</span> <code className="rounded bg-muted px-1 text-xs">{targetedElement.tagName}</code></p>
